...
var $window = $(window),
	...
	mapObject, marker, yPos, coords, TimelineLite, cntFacts = false;
...
var App = {
	...
	onScreen: function() {
		...
		if($('.facts').is_on_screen() && !cntFacts) {
			App.facts();
			cntFacts = 1;
		}
	...
	facts: function() {
		...
		$('.facts li:not(.break)').each(function() {
			...
			$eleNumber.animateNumbers($eleNumber.attr('data-number'), true);
		});
	...
	homeParallax: function() {
		...
		$(window).scroll(function() {
			...
			if(windowWidth > 768) {
				...
				if($('.frst').is_on_screen()) {
					
					yPos = ($(window).scrollTop() - ($('.frst').offset().top + $('.frst').outerHeight())) * 0.5;
					coords = '50% '+ yPos + 'px';
					
					$('.frst').css({ backgroundPosition: coords });
				}
				if($('.secnd').is_on_screen()) {

					yPos = ($(window).scrollTop() - ($('.secnd').offset().top + $('.secnd').outerHeight())) * 0.5;
					coords = '50% '+ yPos + 'px';

					$('.secnd').css({ backgroundPosition: coords });
				}
			}
	...
	parallax: function() {
		...
		if(windowWidth > 768) {
			...
			$('.frst').parallax("70%", -0.6);
			$('.secnd').parallax("70%", -0.6);
		}
	...
	create_map: function(mapContainer) {
		...
		var mapOptions = {
			center: new google.maps.LatLng(55.7436422,37.56778), //37.36456, -121.92852
			zoom: 16,
	...
	create_marker: function() {
		...
		myPin = new google.maps.LatLng(55.7413955,37.5595617);
	...
	create_info: function() {
		...
		boxText.innerHTML = "\
			<div class='marker-label'>\
				Тел. заказов: (495) 223-11-00<br>\
				Доставка : (495) 223-11-00<br>\
				Icq: 585655<br>\
				Заказ: info@studentam.ru<br>\
				Тех. под.: support@studentam.ru<br>\
				Реклама: advert@studentam.ru\
				</div>\
			";
